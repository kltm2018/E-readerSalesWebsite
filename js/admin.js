...
function addThongKe() {
  var dataChart = {
    type: "bar",
    data: {
      labels: ["Haoqing", "Onyx", "Kobo", "Amazon Kindle"],
      datasets: [
        {
...
function addKhungSuaSanPham(masp) {
  ...
  var s =
    ...
    `</th>
            ...
            <tr>
                <td>Mã sản phẩm:</td>
                <td><input type="text" id="maspSua" name="maspSua" value="` +
    sp.MaSP +
    `"></td>
            </tr>
            <tr>
...
if (isset($_POST["submit"])) {
    if (($_FILES["hinhanh"]["type"]=="image/jpeg") ||($_FILES["hinhanh"]["type"]=="image/png") || ($_FILES["hinhanh"]["type"]=="image/jpg") && ($_FILES["hinhanh"]["size"] < 50000) )
    {
        ...
        $tmp = explode(".", $file);
        $duoianh = end($tmp);
        $tenanh = uniqid() . '.' . $duoianh; // Tạo tên ảnh mới để tránh trùng lặp
        $tenfilemoi = "img/products/" . $tenanh;
        
        $i = 1;
        while (file_exists($tenfilemoi)) {
            $tenanh = uniqid() . '_' . $i . '.' . $duoianh;
            $tenfilemoi = "img/products/" . $tenanh;
            $i++;
        }
        ...
        $hinhanh = mysqli_real_escape_string($conn, "/img/products/" . $tenanh);
        $sql = "SELECT HinhAnh FROM sanpham WHERE MaSP = '$masp'";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        $old_hinhanh = $row['HinhAnh'];
        
        // Xóa file ảnh cũ nếu có
        if (file_exists($old_hinhanh)) {
            unlink($old_hinhanh);
        }
