...
window.onload = function () {
  ...
  var tags = ["Haoqing", "Onyx", "Kindle", "Kobo", "Meebook"];
...
function addChiTietToWeb(p) {
  ...
  // Đổi title
  document.title = p.TenSP + " - Thế giới máy đọc sách";
  ...
  if (p.KM.LoaiKM == "GiaReOnline") {
    ...
    area_price.innerHTML =
      `<strong>` + giaTri.toLocaleString() + `&#8363;</strong>` + khuyenmaidiv;
  }
  ...
  switch (sp.KM.LoaiKM) {
    ...
    case "giamgia":
      var span =
        ...
        Number(sp.KM.GiaTriKM).toLocaleString() +
        `</span>`;
      return (
